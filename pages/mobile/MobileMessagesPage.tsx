...
export const MobileMessagesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      ...
      try {
        ...
        // Si providerId dans l'URL, ouvrir cette conversation
        // Sinon, rester sur la liste (ne pas ouvrir de conversation par défaut)
        if (providerId) {
          setActiveChat(providerId);
        }
        // Ne pas ouvrir automatiquement la première conversation
      } catch (error) {
  ...
  // Scroll to bottom seulement quand on envoie un nouveau message (pas au chargement)
  const prevMessagesLength = useRef(messages.length);
  useEffect(() => {
    // Ne scroller que si un nouveau message a été ajouté (et c'est de l'utilisateur)
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Commencez à discuter avec un prestataire
                </p>
              </div>
            ) : (
              <div className="divide-y divide-gray-100">
                {filteredConversations.map((chat) => (
                  ...
                  >
                    ...
                    {chat.unread > 0 && (
                      <div className="w-5 h-5 bg-eveneo-blue text-white text-xs font-bold rounded-full flex items-center justify-center">
                        {chat.unread}
                      </div>
                    )}
                  </div>
